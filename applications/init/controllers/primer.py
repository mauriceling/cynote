...
def primer_output():
    ...
    result['Left primer %GC'] = str(session['lwGC']) + ' to ' + \
                                str(session['lbGC'])
    result['Left primer Tm (C)'] = str(session['lbest']) + ' to ' + \
                                   str(session['lworst'])
    result['Left primer Tm, salt corrected (C)'] = str(session['lbestNa']) + \
                                                   ' to ' + \
                                                   str(session['lworstNa'])
    ...
    if result['Right primer']: 
        ...
        result['Right primer %GC'] = str(session['rwGC']) + ' to ' + \
                                     str(session['rbGC'])
        result['Right primer Tm (C)'] = str(session['rbest']) + ' to ' + \
                                        str(session['rworst'])
        result['Right primer Tm, salt corrected (C)'] = str(session['rbestNa']) + \
                                                        ' to ' + \
                                                        str(session['rworstNa'])
...
def process_primer_tm(result, pos='l'):
    ...
    if result[pos+'len'] < 14:
        ...
        r[pos+'best'] = ((result[pos+'A'] + result[pos+'T'] + \
                ...
                ((result[pos+'G'] + result[pos+'C'] + result[pos+'S']) * 4)
        r[pos+'worst'] = ((result[pos+'A'] + result[pos+'T'] + \
                result[pos+'U'] + result[pos+'W']) * 2) + \
                ((result[pos+'G'] + result[pos+'C'] + result[pos+'S'] + \
                result[pos+'N']) * 4)
        # Tm (salt correction) = (wA+xT)*2 + (yG+zC)*4 -
        #                       16.6*log10(0.050) + 16.6*log10([salt])
    ...
    if result[pos+'U'] == 0 and result[pos+'N'] == 0 and \
       result[pos+'W'] == 0 and result[pos+'S'] == 0:
        ...
        for i in range(len(result[pos+'primer'])):
            try:
                deltaH = deltaH + H[result[pos+'primer'][i] + \
                                    result[pos+'primer'][i+1]]
                deltaG = deltaG + G[result[pos+'primer'][i] + \
                                    result[pos+'primer'][i+1]]
            except IndexError: pass
        saltcon = float(result['salt']) / 1000 + \
                  4 * ((float(result['magnesium']) / 1000) ** 0.5)
        r[pos+'kinTm'] = (298.2*deltaH) / \
                         (deltaH - deltaG + \
                          (1.99 * 298.2 * log(result['pcon']))) + \
...
def design_primer():
    ...
    if form.accepts(request.vars,session):
        ...
        session['left'] = \
            form.vars.sequence.upper()[int(lrange[0]):int(lrange[1])+1]
        session['right'] = \
            form.vars.sequence.upper()[-int(rrange[1]):-int(lrange[0])-1]
        session['result'] = search_primer_pairs(session['left'],
                                                session['right'])
