...
def new_entry():
    ...
    form=SQLFORM(cynotedb.entry,fields=['title','file','keywords','notebook','description'])
...
def notarize_bysystem(username = None):
    ...
    for entry_id in cynotedb().select(cynotedb.entry.id):
        id = entry_id['id']
        cynotedb.comment.insert(author='system', body='notarize by system', entry_id=id)
    cynotedb.commit()
...
def results(): 
    ...
    form = FORM(
            TABLE(*[TR("Result "+str(id['id']), INPUT(_type="checkbox",_name=str(id['id']),value=False,_value='on')) 
                 for id in cynotedb(cynotedb.result.author==session.username).select(cynotedb.result.id)]+\
             [TR("",INPUT(_type="submit",_value="SUBMIT"))]))       
    id_list = []
    if form.accepts(request.vars,session):
        session['form_vars'] = form.vars
        option_checked = [id['id'] for id['id']in form.vars.keys() if form.vars[id['id']]]
...
def my_exporter(): 
    ...
    return str(cynotedb(cynotedb.notebook.name == 'test').select(cynotedb.entry.ALL))
